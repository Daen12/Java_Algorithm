...
public class Q2887_행성터널verDFS_platinum5 {
    static class planet {
        int x, y, z;
    ...
    // public static void Permutation(int depth, int cost, int prev) {
    // if (cost >= minCost)
    // return;
    // if (depth == N) {
    // // int sum = 0;
    // // for (int i = 0; i < N - 1; i++) {
    // // sum += distance(planets[sel[i]], planets[sel[i + 1]]);
    // // // tunnels[i] = new tunnel(sel[i], sel[i+1], distance(planets[sel[i]],
    // // // planets[sel[i+1]]));
    // // } // 뽑힌 순열에 대해 터널 정보를 저장
    ...
    // isUsed[i] = true;
    // sel[depth] = i;
    // Permutation(depth + 1, cost + distance(planets[prev], planets[i]), i);
    // isUsed[i] = false;
    // }
    // }
    // }
